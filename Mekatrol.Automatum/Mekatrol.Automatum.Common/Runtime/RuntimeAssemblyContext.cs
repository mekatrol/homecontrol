...
namespace Mekatrol.Automatum.Common.Runtime;

public class RuntimeAssemblyContext : AssemblyLoadContext
{
    private readonly AssemblyDependencyResolver _resolver;
    private readonly Action _unload;

    // Make constructor private, must be instantiated through static method
    private RuntimeAssemblyContext(string pluginPath, Action unload) : base(isCollectible: true)
    {
        _resolver = new(pluginPath);
        _unload = unload;
    }

    public static (RuntimeAssemblyContext, Assembly?, IList<string>) LoadAndCompile(
        string pluginPath,
        string sourceCode,
    ...
    {
        var context = new RuntimeAssemblyContext(pluginPath, unload);
    ...
    {
        var scriptCompiler = new RuntimeCompiler();
