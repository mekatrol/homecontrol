...
export const getTimeWithMeridiem = (dateTime: Date | undefined = undefined): string => {
  const dt = dateTime ?? new Date();
  return getZeroPadded(getHours12Hour(dt)) + ':' + getZeroPadded(dt.getMinutes()) + ' ' + getMeridiem(dt);
};
